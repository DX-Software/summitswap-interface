...
function TabBrowseAllCollection({ whitelabelNftFactory }: Props) {
  const [page, setPage] = useState(1)
  const [search, setSearch] = useState<string>()
  const debouncedSearch = useDebounce(search, 1000)
  const { setWhitelabelNtId } = useWhitelabelNftContext()

  const whitelabelNftCollections = useWhitelabelNftCollections(page, PER_PAGE, debouncedSearch, [
    Phase.Pause,
    Phase.Public,
    Phase.Whitelist,
  ])
...
export default React.memo(TabBrowseAllCollection)

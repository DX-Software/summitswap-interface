...
function TabWhitelistPhaseCollection({ whitelabelNftFactory }: Props) {
  const [page, setPage] = useState(1)
  const [search, setSearch] = useState<string>()
  const debouncedSearch = useDebounce(search, 1000)
  const { setWhitelabelNtId } = useWhitelabelNftContext()

  const whitelabelNftCollections = useWhitelabelNftCollections(page, PER_PAGE, debouncedSearch, [Phase.Whitelist])

  if (whitelabelNftFactory.isLoading) {
    return null
  }

  const totalWhitelabelNft = whitelabelNftFactory.data?.totalWhitelabelNftWhitelistPhase?.toNumber() || 0
...
export default React.memo(TabWhitelistPhaseCollection)

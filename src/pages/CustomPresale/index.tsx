...
export default function CustomPresale() {
  ...
  useEffect(() => {
    async function checkTokenIsApproved() {
      const aprrovedAmount: BigNumber = await tokenContract?.allowance(account, factoryAddress)
      if (aprrovedAmount.eq(BigNumber.from(maxHexValue))) {
    ...
    onSubmit: async (values: Values) => {
      ...
      try {
        const receipt = await factoryContract.createPresale(
          ...
          values.feeType === FieldValues.feeTypeOnlyBnb ? 0 : 1,
          values.refundType === FieldValues.refundTypeRefund ? 0 : 1,
          values.isWhitelistEnabled === FieldValues.whitelistEnable,
          {
  ...
  const onApproveTokenHandler = useCallback(async () => {
    ...
    try {
      setIsLoading(true)
      const receipt = await tokenContract?.approve(factoryAddress, maxHexValue)
      await library.waitForTransaction(receipt.hash)
      setIsLoading(false)
      setIsFactoryApproved(true)
    } catch (err) {
  ...
  return account ? (
    <>
      ...
      {selectedToken && !presaleAddress ? (
        ...
        <ColumnFlatCenter>
          {presaleAddress && (
            <Text bold fontSize="23px" mb="5px">
              Token Presale at: {presaleAddress}
            </Text>
          )}
        </ColumnFlatCenter>
      )}
    ...
    <Button m={100} style={{ fontFamily: 'Poppins' }} onClick={onPresentConnectModal}>
